...
#
# Amnesia.exe is 32-bit, so we vendor the x86 loader. ThirteenAG ships the x86
# build in Ultimate-ASI-Loader.zip (the x64 build is Ultimate-ASI-Loader_x64.zip).
# We extract dinput8.dll and vendor it; install.cmd copies it to the exe dir as
# wininet.dll (the import slot Amnesia loads ASI plugins through).
#
...
try {
    ...
    $readme = @(
        ...
        '',
        'install.cmd copies `dinput8.dll` to the Amnesia exe dir as `wininet.dll` (the import',
        'slot Amnesia loads ASI plugins through).',
        '',
        ...
        "loader's own filename, and we deploy it as `wininet.dll`, while the windower needs a",
